...
public class changeusername extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (ud.checkExit((String) request.getParameter("username")) == 1) {
            request.setAttribute("ms", "Username đã tồn tại");
            request.getRequestDispatcher("changeusername.jsp").forward(request, response);
        } else {
            HttpSession ss = request.getSession();
            User u = (User) ss.getAttribute("user");
            ud.updateUser(u.getId(), u.getName(), (String) request.getParameter("username"), u.getEmail());
            ss.setAttribute("user", ud.getUser(u.getId()));
