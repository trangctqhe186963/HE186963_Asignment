...
public class forgotpass extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String userormail = (String) request.getParameter("nameoremail");
        UserDAO ud = new UserDAO();
        if (ud.checkExit(userormail) == 1) {
            HttpSession ss = request.getSession();
            ss.setAttribute("tempuser", ud.getUser(userormail));
            Send sm = new Send();
            String code = sm.getRandom();
            User u = ud.getUser(userormail);
            boolean test = sm.sendEmailchangeinfor(u.getEmail(), code, "33");
